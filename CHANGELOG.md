...
- `AgentPlatformSlackRateLimited` and `AgentPlatformSlackCallGivenUp`: notify team Bumblebee when Slack rate-limits klaus-gateway's Web API calls for ten minutes running, and as soon as the gateway gives up a rate-limited call (`klaus_gateway_slack_rate_limited_total`; giantswarm/klaus-gateway#313).
